...
const CoDataPublishForKafka = require('./app/components/dataPublishForKafka');
const CoDataReceiveForKafka = require('./app/components/dataReceiveForKafka');
...
app.configure('production|development', 'dataSourceConnector', () => {
    ...
    app.load(CoDataPublishForKafka, {});
...
app.configure('production|development', 'connector', () => {
    ...
    app.load(CoDataReceiveForKafka, {});
    app.set('errorHandler', (err, msg, resp, session, next) => {
        console.error(err.stack);
    });
});
